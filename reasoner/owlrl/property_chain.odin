...
@(private) Path_State :: struct { start, endpoint: term.Term_ID }

@(private) append_path_state :: proc(items: ^[dynamic]Path_State, value: Path_State, max_states: int) -> Property_Chain_Error_Code {
	for item in items^ do if item.start == value.start && item.endpoint == value.endpoint do return .None
...
@(private) add_property_chain_fact :: proc(profile: ^Profile, target: ^store.Store, subject, predicate, object: term.Term_ID, remaining: int, added_count: ^int) -> (Property_Chain_Error_Code, store.Error_Code) {
	...
	if rdf.validate_triple_structure({subject_term, predicate_term, object_term}) != .None do return .None, .None
...
@(private) emit_property_chains :: proc(profile: ^Profile, target: ^store.Store, options: Property_Chain_Options, remaining: int) -> (added_count: int, error: Property_Chain_Error_Code, list_error: List_Error_Code, store_error: store.Error_Code) {
	list: List
	init_list(&list)
	...
	for declaration_index in 0..<store.fact_count(target) {
		...
		for item_index in 0..<list_count(&list) {
			...
			if property_term.kind != .IRI do return added_count, .Invalid_Chain_Property, .None, .None
		}
		clear(&frontier)
		first_property, _ := list_item_at(&list, 0)
		for fact_index in 0..<store.fact_count(target) {
			_, fact, _, fact_found := store.fact_at(target, fact_index)
			if !fact_found do return added_count, .Store_Error, .None, .Invalid_Fact
			if fact.predicate != first_property do continue
			if state_error := append_path_state(&frontier, {start = fact.subject, endpoint = fact.object}, options.max_path_states); state_error != .None do return added_count, state_error, .None, .None
		}
		for property_index in 1..<list_count(&list) {
			...
			for state in frontier {
				for fact_index in 0..<store.fact_count(target) {
					...
					if state_error := append_path_state(&next_frontier, {start = state.start, endpoint = fact.object}, options.max_path_states); state_error != .None do return added_count, state_error, .None, .None
				}
			}
		...
		for state in frontier {
			phase_error, insert_error := add_property_chain_fact(profile, target, state.start, declaration.subject, state.endpoint, remaining, &added_count)
...
// materialize_property_chains alternates static rules with strict-RDF
// owl:propertyChainAxiom expansion until joint fixpoint. Chains must contain
// at least two IRI predicates; all work is committed only after success.
