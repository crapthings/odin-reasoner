...
@(private) add_union_type :: proc(profile: ^Profile, target: ^store.Store, subject, class: term.Term_ID, remaining: int, added_count: ^int, generalized_heads: bool, provenance: ^Closure_Provenance = nil, declaration_id: store.Fact_ID = store.INVALID_FACT_ID, list: ^List = nil, supports: []store.Fact_ID = nil) -> (Union_Error_Code, store.Error_Code) {
	...
	if added {
		...
		if provenance != nil {
			...
			if fact_id == store.INVALID_FACT_ID || declaration_id == store.INVALID_FACT_ID || list == nil || !append_list_derivation(provenance, fact_id, OWL_RL_CLS_UNI, declaration_id, list, supports) do return .Store_Error, .Out_Of_Memory
		}
	}
	return .None, .None
}
...
@(private) add_union_subclass :: proc(profile: ^Profile, target: ^store.Store, subclass, superclass: term.Term_ID, remaining: int, added_count: ^int, generalized_heads: bool, provenance: ^Closure_Provenance = nil, declaration_id: store.Fact_ID = store.INVALID_FACT_ID, list: ^List = nil) -> (Union_Error_Code, store.Error_Code) {
	...
	if added {
		...
		if provenance != nil {
			fact_id := store.id_for_fact(target, fact)
			if fact_id == store.INVALID_FACT_ID || declaration_id == store.INVALID_FACT_ID || list == nil || !append_list_derivation(provenance, fact_id, OWL_RL_SCM_UNI, declaration_id, list, nil) do return .Store_Error, .Out_Of_Memory
		}
	}
	return .None, .None
}
...
@(private) emit_union :: proc(profile: ^Profile, target: ^store.Store, options: Union_Options, remaining: int, provenance: ^Closure_Provenance = nil) -> (added_count: int, error: Union_Error_Code, list_error: List_Error_Code, store_error: store.Error_Code) {
	list: List
	init_list(&list)
