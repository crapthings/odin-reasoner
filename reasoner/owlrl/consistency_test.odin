...
test_materialize_checked_reports_each_supported_false_rule :: proc(t: ^testing.T) {
	...
	p1, p2, p3, p4, p5, p6, p7, p8, p9, s, o := rdf.iri("urn:p1"), rdf.iri("urn:p2"), rdf.iri("urn:p3"), rdf.iri("urn:p4"), rdf.iri("urn:p5"), rdf.iri("urn:p6"), rdf.iri("urn:p7"), rdf.iri("urn:p8"), rdf.iri("urn:p9"), rdf.iri("urn:s"), rdf.iri("urn:o")
	...
	qualified_class, qualified_member := rdf.iri("urn:qualified-class"), rdf.iri("urn:qualified-member")
	qualified_zero, qualified_subject := rdf.iri("urn:qualified-zero"), rdf.iri("urn:qualified-subject")
	add_consistency_fact(t, &target, {qualified_zero, rdf.iri(OWL_MAX_QUALIFIED_CARDINALITY), rdf.typed_literal("0", XSD_NON_NEGATIVE_INTEGER)})
	add_consistency_fact(t, &target, {qualified_zero, rdf.iri(OWL_ON_PROPERTY), p8})
	add_consistency_fact(t, &target, {qualified_zero, rdf.iri(OWL_ON_CLASS), qualified_class})
	add_consistency_fact(t, &target, {qualified_subject, type, qualified_zero})
	add_consistency_fact(t, &target, {qualified_subject, p8, qualified_member})
	add_consistency_fact(t, &target, {qualified_member, type, qualified_class})
	qualified_thing_zero, qualified_thing_subject := rdf.iri("urn:qualified-thing-zero"), rdf.iri("urn:qualified-thing-subject")
	add_consistency_fact(t, &target, {qualified_thing_zero, rdf.iri(OWL_MAX_QUALIFIED_CARDINALITY), rdf.typed_literal("0", XSD_NON_NEGATIVE_INTEGER)})
	add_consistency_fact(t, &target, {qualified_thing_zero, rdf.iri(OWL_ON_PROPERTY), p9})
	add_consistency_fact(t, &target, {qualified_thing_zero, rdf.iri(OWL_ON_CLASS), rdf.iri(OWL_THING)})
	add_consistency_fact(t, &target, {qualified_thing_subject, type, qualified_thing_zero})
	add_consistency_fact(t, &target, {qualified_thing_subject, p9, rdf.literal("forbidden")})
	...
	testing.expect_value(t, count_kind(&report, .Max_Qualified_Cardinality_Zero), 2)
	for index in 0..<violation_count(&report) {
		violation, found := violation_at(&report, index)
		...
		if found && violation.kind == .Max_Qualified_Cardinality_Zero do testing.expect(t, violation.support_count == 5 || violation.support_count == 6)
	}
}
